...
type Product = StoreProduct & {
  description: string;
  variants?: Variant[];
  images?: { id: number; url: string; alt?: string | null; colour?: string | null }[];
};
  ...
  const sizes = useMemo(
    () =>
      Array.from(
        new Set(
          variants
            .filter((v) => !colour || v.colour === colour)
            .map((v) => v.size)
        )
      ),
    [variants, colour]
  ...
  return (
    <div className="variant-panel">
      <div className="price-row large">
        ...
        {hasSale && <del>£{regular.toFixed(2)}</del>}
      </div>
          ...
          <div className="variant-options">
            {colours.map((c) => (
              <button
                type="button"
                key={c}
                className={colour === c ? "variant-choice active" : "variant-choice"}
      ...
      {needsVariant && (
        <div className="variant-group">
          <p><strong>Size:</strong> {size || "Select a size"}</p>
          <div className="variant-options">
            {sizes.map((s) => {
              const v = variants.find((x) => x.colour === colour && x.size === s);
              const sold = !v || v.stock < 1;
              return (
                <button
                  type="button"
                  key={s}
                  disabled={sold}
                  className={size === s ? "variant-choice active" : "variant-choice"}
                  onClick={() => setSize(s)}
                >
                  {s}{sold ? " · Sold out" : ""}
                </button>
      ...
      <button className="primary-button variant-add" type="button" disabled={!canAdd} onClick={add}>
        {added ? "Added to bag" : needsVariant && !selected ? "Choose size" : "Add to bag"}
      </button>

      {selected && (
        <p className="stock-note">
          {selected.stock > 5 ? "In stock" : selected.stock > 0 ? `Only ${selected.stock} left` : "Sold out"} · SKU {selected.sku}
        </p>
      )}
    </div>
  );
